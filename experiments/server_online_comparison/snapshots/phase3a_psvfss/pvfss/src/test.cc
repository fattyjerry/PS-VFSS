...
void PrintClientBenchJson(const BenchmarkParams &params, const BenchmarkResult &result) {
    ...
    std::cout << "[CLIENT_BENCH_JSON] {"
              ...
              << "\"recipient_processing_ns\":" << result.reconstruction_us * 1000 << ","
              << "\"correctness\":" << (ok ? "true" : "false") << ","
              << "\"output_type\":\"matching_signal_indices\","
              << "\"error_class\":\"" << (ok ? "" : "recovered_result_mismatch") << "\","
              << "\"scheme_specific_parameters\":\"vdpf_bit_length=64;T=" << params.T
              << ";Nr=not_exposed\"}" << std::endl;
}
}

namespace CharacterTest {
    BenchmarkResult RunBenchmarkCase(int party, NetIO *io, BenchmarkParams params) {
        ...
        // Use a dedicated channel for BTGen.  The initial seed exchange above
        // uses the legacy EMP FILE* stream; keeping Paillier's bidirectional
        // framed traffic on a fresh connection avoids cross-phase stdio state.
        NetIO *btio = new NetIO(party == 1 ? nullptr : "127.0.0.1", port + 1, false);
        setvbuf(btio->stream, nullptr, _IONBF, 0);
        btio->sync();
        BTGen(party, btio, fa, fb, fch, fcl);
        delete btio;
    ...
    void ServerTest0(int argc, char **argv) {
        ...
        // NetIO's default 1 MiB stdio buffer makes fread wait for a full
        // buffer even when the peer has sent one framed BN message.  The
        // protocol is length-delimited, so use exact unbuffered socket I/O.
        setvbuf(io->stream, nullptr, _IOFBF, 1);
