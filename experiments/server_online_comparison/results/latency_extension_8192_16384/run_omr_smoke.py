...
cmd = [str(binary), "--bench", "omrp1", "--threads", "1", "--N", "8192",
...
(HERE / "logs/omr_degree16384_N8192_smoke.log").write_text(p.stdout)
if p.returncode:
...
(HERE / "omr_degree16384_smoke.json").write_text(json.dumps(rows[-1], indent=2, sort_keys=True) + "\n")
