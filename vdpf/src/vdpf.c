	...
	unsigned char *k1)
{
	...
	while (!didFinish)
	{
		...
		uint64_t sFinal0 = field61_from_u128(convert(&seeds0[size]));
		uint64_t sFinal1 = field61_from_u128(convert(&seeds1[size]));
		uint64_t baseCW = field61_add(beta, field61_sub(sFinal1, sFinal0));
		uint64_t lastCW = bits1[size] == 0 ? baseCW : field61_neg(baseCW);
		...
		memset(&k0[INDEX_LASTCW], 0, 16);
		memcpy(&k0[INDEX_LASTCW], &lastCW, sizeof(lastCW));
	...
	uint8_t *proof)
{
	...
	for (int l = 0; l < inl; l++)
	{
		...
		uint64_t res = field61_from_u128(convert(&seeds[size]));

		if (bits[size] == 1)
		{
			// correction word
			uint64_t lastCW;
			memcpy(&lastCW, &k[INDEX_LASTCW], sizeof(lastCW));
			res = field61_add(res, lastCW);
		}
		if (b)
			res = field61_neg(res);

		// Retain the historical 16-byte output stride for ABI compatibility.
		uint128_t encoded = res;
		memcpy(&out[l * sizeof(uint128_t)], &encoded, sizeof(encoded));
	}
	...
	uint8_t *proof)
{
	...
	for (int i = 0; i < numLeaves; i++)
	{
		...
		uint64_t res = field61_from_u128(convert(&seeds[index]));
